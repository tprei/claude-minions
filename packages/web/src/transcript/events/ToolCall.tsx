  ...
  return (
    <div className="my-0.5">
      <button
        ...
        className="flex items-center gap-2 text-xs text-zinc-400 hover:text-zinc-200 transition-colors w-full text-left"
      >
        <span>{icon}</span>
        <span className="font-mono text-zinc-300">{event.toolName}</span>
        <span className="text-zinc-500 truncate flex-1">{event.summary}</span>
        <span className={cx("shrink-0 transition-transform", expanded ? "rotate-90" : "")}>›</span>
      </button>
      {expanded && (
        <pre className="mt-1 text-[11px] text-zinc-400 bg-zinc-900 rounded p-2 border border-border overflow-x-auto">
