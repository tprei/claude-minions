...
const SOURCE_COLORS: Record<NonNullable<UserMessageEvent["source"]>, string> = {
  ...
  completion: "bg-zinc-700 text-zinc-300",
};
...
export function UserMessage({ event }: Props) {
  ...
  return (
    <div className="flex flex-col items-end gap-1 py-1">
      ...
      <div className="max-w-[80%] bg-blue-900/40 border border-blue-700/40 rounded-xl px-3 py-2 text-sm text-zinc-100 whitespace-pre-wrap">
        {event.text}
      </div>
      {event.attachments && event.attachments.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {event.attachments.map((a, i) => (
            <span key={i} className="pill bg-zinc-700 text-zinc-300 text-[10px]">
              {a.name}
            </span>
          ))}
        </div>
