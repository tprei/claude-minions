      ...
      <div className="px-2 flex flex-col gap-0.5">
        ...
        {features.has("runtime-overrides") && (
          <button onClick={onOpenRuntime} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs text-fg-muted hover:text-fg hover:bg-bg-elev transition-colors">
            ...
            {admissionUnlimited && (
              <span
                data-testid="admission-unlimited-badge"
                className="pill bg-amber-900/40 text-amber-300 text-[10px]"
                title="Admission caps disabled — engine will accept any number of concurrent sessions"
              >
                unlimited
              </span>
            )}
