...
export function autoCommitHandler(ctx: EngineContext): (session: Session) => Promise<void> {
  return async (session: Session) => {
    ...
    const git = simpleGit(session.worktreePath).env({
      ...process.env,
      GIT_AUTHOR_NAME: "minions-engine",
      GIT_AUTHOR_EMAIL: "engine@minions.local",
      GIT_COMMITTER_NAME: "minions-engine",
      GIT_COMMITTER_EMAIL: "engine@minions.local",
    });
